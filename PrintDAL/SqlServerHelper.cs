...
namespace PrintDAL
{
    ...
    public class SqlServerHelper : IDbHelper, IDisposable
    {
        ...
        /// 创建系统异常日志
        /// </summary>
        protected SysLogHelper Logger = new SysLogHelper("SQLServerLog");
            ...
            if (db == null)
            {
                if (ConfigHelper.GetAppSettings("ConStringEncrypt") == "true")//判断是否加密
                    ...
                    try
                    {
                        ...
                        if (db == null)
                        {
                            Logger.WriteLog("db null\r\n");
                        }
                    }
                    catch (Exception e)
                    {
                        Logger.WriteLog("初始化数据连接失败\r\n" + e.Message + "\r\n");
                    }
        ...
        /// <param name="param">参数化</param>
        /// <returns>object</returns>
        public int ExecuteBySql(string sql, SqlParam[] param)
        {
            int num = 0;
            try
            {
        ...
        /// <returns></returns>
        public int ExecuteBySqls(List<string> listSql, object[] param)
        {
            int num = 0;
                ...
                using (DbConnection connection = this.GetDatabase().CreateConnection())
                {
                    ...
                    try
                    {
                        for (int i = 0; i < listSql.Count; i++)
                        {
                            ...
                            sql_Log.Append(strSql + "\r\n");
                            if (strSql != null)
                            {
                                if (param == null)
                                {

                                    DbCommand sqlStringCommand = this.db.GetSqlStringCommand(strSql);
       ...
       return this.GetDatabase().ExecuteDataSet(CommandType.Text, strSql).Tables[0];
        }

        //返回执行行数 参数为字符串
        private object GetObjectValue(string strSql)
        {
            try
            {
                dbCommand = this.GetDatabase().GetSqlStringCommand(strSql);
                return db.ExecuteScalar(dbCommand);
            }
            catch (Exception e)
            {
        ...
        /// <summary>
        /// 摘要:
        ///     数据分页接口 返回数据集字段名称区分大小写
        /// 参数：
        ///     sql：传入要执行sql语句 
        ///     orderField：排序字段 如有多个字段 “,”分割 如 字段A,字段B
        ///     orderType：排序类型  每种排序类型对应一个排序字段 如   DESC,ASC
        ///     pageIndex：当前页
        ///     pageSize：页大小
        ///     count：返回查询条数
        /// </summary>
        public DataTable GetPageList(string sql, string orderField, string orderType, int pageIndex, int pageSize, ref int count)
        {
            ...
            try
            {
                string orderStr = "";//orderField + " " + orderType;

                if (!string.IsNullOrEmpty(orderField))
                {
                    string[] strField = (orderField.Replace(" ", "")).Split(',');
                    string[] strType = (orderType.Replace(" ", "")).Split(',');

                    for (int i = 0; i < strField.Length; i++)
                    {
                        orderStr += " " + strField[i] + " " + strType[i] + ",";
                    }
                ...
                count = Convert.ToInt32(this.GetObjectValue("Select Count(1) From (" + sql + ") As t"));
                return GetQuery(sb.ToString());
            }
            catch (Exception e)
            {
        ...
        public DataTable QueryDataTable(string TargetTable, bool isAll = false)
        {
        ...
        public DataTable GetProcData(string ProcName, SqlParameter[] param)
        {


            DbCommand cmd = this.GetDatabase().GetStoredProcCommand(ProcName);
            cmd.CommandType = CommandType.StoredProcedure;
            DataTable dt = new DataTable();
            if (param != null)
                foreach (SqlParameter paramt in param)
            ...
            using (DbConnection connection = this.db.CreateConnection())
            {
                ...
                try
                {
                    dt = this.db.ExecuteDataSet(cmd).Tables[0];
                }
                catch (Exception exception)
                {
            ...
            //storedProcCommand.Parameters.Add()

            //return null;
        }


        public int ExecuteProcData(string ProcName, SqlParameter[] param)
        {
            ...
            using (DbConnection connection = this.db.CreateConnection())
            {
                ...
                try
                {
                    result = this.db.ExecuteNonQuery(cmd);
                }
                catch (Exception exception)
                {
                    Logger.WriteLog("-----------执行存储过程-----------\r\n" + ProcName + "\r\n" + exception.Message + "\r\n");
            ...
            try
            {
                ...
                foreach (string str in ht.Keys)
                {
                    if (str.StartsWith("OUT_"))
                    {
                        ...
                        object parameterValue = this.db.GetParameterValue(storedProcCommand, "@" + str);
        ...
        /// </summary>
        /// <returns></returns>
        public bool Submit_AddOrEdit(string tableName, string pkName, string pkVal, Hashtable ht, bool IsEdit = true)
        {
            if (!string.IsNullOrEmpty(pkVal) && IsEdit)
            {
        ...
        /// <param name="pkVal">字段值</param>
        /// <returns></returns>
        public int IsExist(string tableName, string pkName, string pkVal, string filter = "")
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("Select Count(1) from " + tableName + "");
            strSql.Append(" where 1=1 and " + pkName + " = @" + pkName + " ");
